    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BgpCapabilityParsingError::ReservedExperimentalCode(v) => {
                write!(f, "reserved BGP capability code for experimental use: {}", v)
            }
        }
    }
}
    ...
    match BgpCapabilityType::from_u8(*capability_code) {
        ...
        None => {
            if [4, 66, 128, 129, 130, 131, 184, 185].contains(capability_code) {
                ...
                Err(BgpCapabilityParsingError::ReservedExperimentalCode(*capability_code))
            } else {
...
mod tests {
    ...
    fn test_parsing_capability() {
        ...
        // reserved
        code = 0;
        assert_eq!(parse_capability(&code), Err(BgpCapabilityParsingError::ReservedCode(code)));
        code = 255;
        assert_eq!(parse_capability(&code), Err(BgpCapabilityParsingError::ReservedCode(code)));

        // deprecated
        for code in [4, 66, 128, 129, 130, 131, 184, 185] {
            assert_eq!(parse_capability(&code), Err(BgpCapabilityParsingError::DeprecatedCode(code)));
        }

        // unassigned
        for code in 10..=63 {
            assert_eq!(parse_capability(&code), Err(BgpCapabilityParsingError::Unassigned(code)));
        }
        for code in 74..=127 {
            assert_eq!(parse_capability(&code), Err(BgpCapabilityParsingError::Unassigned(code)));
        }
        ...
        // valid capabilities
        code = 1;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::MULTIPROTOCOL_EXTENSIONS_FOR_BGP_4));
        code = 2;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::ROUTE_REFRESH_CAPABILITY_FOR_BGP_4));
        code = 3;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::OUTBOUND_ROUTE_FILTERING_CAPABILITY));
        code = 5;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::EXTENDED_NEXT_HOP_ENCODING));
        code = 6;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::BGP_EXTENDED_MESSAGE));
        code = 7;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::BGPSEC_CAPABILITY));
        code = 8;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::MULTIPLE_LABELS_CAPABILITY));
        code = 9;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::BGP_ROLE));

        code = 64;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::GRACEFUL_RESTART_CAPABILITY));
        code = 65;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::SUPPORT_FOR_4_OCTET_AS_NUMBER_CAPABILITY));
        code = 67;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::SUPPORT_FOR_DYNAMIC_CAPABILITY));
        code = 68;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::MULTISESSION_BGP_CAPABILITY));
        code = 69;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::ADD_PATH_CAPABILITY));
        code = 70;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::ENHANCED_ROUTE_REFRESH_CAPABILITY));
        code = 71;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::LONG_LIVED_GRACEFUL_RESTART_CAPABILITY));
        code = 72;
        assert_eq!(parse_capability(&code), Ok(BgpCapabilityType::ROUTING_POLICY_DISTRIBUTION));
        code = 73;
